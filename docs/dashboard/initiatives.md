...
- **All Initiatives** (`/`) groups initiative tiles under each program's name (skipping any program marked hidden), with a per-program progress bar showing the average progress across that program's initiatives, followed by a **Standalone** group for initiatives with no `programId`.
