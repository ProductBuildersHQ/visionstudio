...
2. **Database-first orchestration** — prism-build. Started as a Dolt DB with
...
Meanwhile the UI story is fragmented: prism-build has a Go-template dashboard
...
- **Backend:** the Dolt DB and Go service layer (moved from prism-build) grow
...
- **Per-repo tools stay standalone.** devfolio, prism-maturity, prism-roadmap,
  and a slimmed prism-build keep working against local files and emitting
...
1. **G1 — IR contract layer.** `visionstudio/pkg/ir` composes the domain IRs by
   ...
   prism-roadmap, prism-build). Source modules remain the source of truth.
2. **G2 — prism-build split.** prism-build keeps its structs/JSON IR and
...
4. **G4 — Composed frontend.** React panels (execution dashboard, spend
   ...
   prism-build Phase 4 (RMI-PRISMCONTROL-116..119).
