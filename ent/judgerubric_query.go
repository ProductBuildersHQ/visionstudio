// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"database/sql/driver"
	"fmt"
	"math"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/ProductBuildersHQ/visionstudio/ent/judgeresult"
	"github.com/ProductBuildersHQ/visionstudio/ent/judgerubric"
	"github.com/ProductBuildersHQ/visionstudio/ent/predicate"
	"github.com/ProductBuildersHQ/visionstudio/ent/specworkflow"
)

// JudgeRubricQuery is the builder for querying JudgeRubric entities.
type JudgeRubricQuery struct {
	config
	ctx          *QueryContext
	order        []judgerubric.OrderOption
	inters       []Interceptor
	predicates   []predicate.JudgeRubric
	withWorkflow *SpecWorkflowQuery
	withResults  *JudgeResultQuery
	withFKs      bool
	// intermediate query (i.e. traversal path).
	sql  *sql.Selector
	path func(context.Context) (*sql.Selector, error)
}

// Where adds a new predicate for the JudgeRubricQuery builder.
func (_q *JudgeRubricQuery) Where(ps ...predicate.JudgeRubric) *JudgeRubricQuery {
	_q.predicates = append(_q.predicates, ps...)
	return _q
}

// Limit the number of records to be returned by this query.
func (_q *JudgeRubricQuery) Limit(limit int) *JudgeRubricQuery {
	_q.ctx.Limit = &limit
	return _q
}

// Offset to start from.
func (_q *JudgeRubricQuery) Offset(offset int) *JudgeRubricQuery {
	_q.ctx.Offset = &offset
	return _q
}

// Unique configures the query builder to filter duplicate records on query.
// By default, unique is set to true, and can be disabled using this method.
func (_q *JudgeRubricQuery) Unique(unique bool) *JudgeRubricQuery {
	_q.ctx.Unique = &unique
	return _q
}

// Order specifies how the records should be ordered.
func (_q *JudgeRubricQuery) Order(o ...judgerubric.OrderOption) *JudgeRubricQuery {
	_q.order = append(_q.order, o...)
	return _q
}

// QueryWorkflow chains the current query on the "workflow" edge.
func (_q *JudgeRubricQuery) QueryWorkflow() *SpecWorkflowQuery {
	query := (&SpecWorkflowClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(judgerubric.Table, judgerubric.FieldID, selector),
			sqlgraph.To(specworkflow.Table, specworkflow.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, judgerubric.WorkflowTable, judgerubric.WorkflowColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryResults chains the current query on the "results" edge.
func (_q *JudgeRubricQuery) QueryResults() *JudgeResultQuery {
	query := (&JudgeResultClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(judgerubric.Table, judgerubric.FieldID, selector),
			sqlgraph.To(judgeresult.Table, judgeresult.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, judgerubric.ResultsTable, judgerubric.ResultsColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// First returns the first JudgeRubric entity from the query.
// Returns a *NotFoundError when no JudgeRubric was found.
func (_q *JudgeRubricQuery) First(ctx context.Context) (*JudgeRubric, error) {
	nodes, err := _q.Limit(1).All(setContextOp(ctx, _q.ctx, ent.OpQueryFirst))
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nil, &NotFoundError{judgerubric.Label}
	}
	return nodes[0], nil
}

// FirstX is like First, but panics if an error occurs.
func (_q *JudgeRubricQuery) FirstX(ctx context.Context) *JudgeRubric {
	node, err := _q.First(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
}

// FirstID returns the first JudgeRubric ID from the query.
// Returns a *NotFoundError when no JudgeRubric ID was found.
func (_q *JudgeRubricQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
	if ids, err = _q.Limit(1).IDs(setContextOp(ctx, _q.ctx, ent.OpQueryFirstID)); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{judgerubric.Label}
		return
	}
	return ids[0], nil
}

// FirstIDX is like FirstID, but panics if an error occurs.
func (_q *JudgeRubricQuery) FirstIDX(ctx context.Context) string {
	id, err := _q.FirstID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
}

// Only returns a single JudgeRubric entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one JudgeRubric entity is found.
// Returns a *NotFoundError when no JudgeRubric entities are found.
func (_q *JudgeRubricQuery) Only(ctx context.Context) (*JudgeRubric, error) {
	nodes, err := _q.Limit(2).All(setContextOp(ctx, _q.ctx, ent.OpQueryOnly))
	if err != nil {
		return nil, err
	}
	switch len(nodes) {
	case 1:
		return nodes[0], nil
	case 0:
		return nil, &NotFoundError{judgerubric.Label}
	default:
		return nil, &NotSingularError{judgerubric.Label}
	}
}

// OnlyX is like Only, but panics if an error occurs.
func (_q *JudgeRubricQuery) OnlyX(ctx context.Context) *JudgeRubric {
	node, err := _q.Only(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// OnlyID is like Only, but returns the only JudgeRubric ID in the query.
// Returns a *NotSingularError when more than one JudgeRubric ID is found.
// Returns a *NotFoundError when no entities are found.
func (_q *JudgeRubricQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
	if ids, err = _q.Limit(2).IDs(setContextOp(ctx, _q.ctx, ent.OpQueryOnlyID)); err != nil {
		return
	}
	switch len(ids) {
	case 1:
		id = ids[0]
	case 0:
		err = &NotFoundError{judgerubric.Label}
	default:
		err = &NotSingularError{judgerubric.Label}
	}
	return
}

// OnlyIDX is like OnlyID, but panics if an error occurs.
func (_q *JudgeRubricQuery) OnlyIDX(ctx context.Context) string {
	id, err := _q.OnlyID(ctx)
	if err != nil {
		panic(err)
	}
	return id
}

// All executes the query and returns a list of JudgeRubrics.
func (_q *JudgeRubricQuery) All(ctx context.Context) ([]*JudgeRubric, error) {
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryAll)
	if err := _q.prepareQuery(ctx); err != nil {
		return nil, err
	}
	qr := querierAll[[]*JudgeRubric, *JudgeRubricQuery]()
	return withInterceptors[[]*JudgeRubric](ctx, _q, qr, _q.inters)
}

// AllX is like All, but panics if an error occurs.
func (_q *JudgeRubricQuery) AllX(ctx context.Context) []*JudgeRubric {
	nodes, err := _q.All(ctx)
	if err != nil {
		panic(err)
	}
	return nodes
}

// IDs executes the query and returns a list of JudgeRubric IDs.
func (_q *JudgeRubricQuery) IDs(ctx context.Context) (ids []string, err error) {
	if _q.ctx.Unique == nil && _q.path != nil {
		_q.Unique(true)
	}
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryIDs)
	if err = _q.Select(judgerubric.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}

// IDsX is like IDs, but panics if an error occurs.
func (_q *JudgeRubricQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
	return ids
}

// Count returns the count of the given query.
func (_q *JudgeRubricQuery) Count(ctx context.Context) (int, error) {
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryCount)
	if err := _q.prepareQuery(ctx); err != nil {
		return 0, err
	}
	return withInterceptors[int](ctx, _q, querierCount[*JudgeRubricQuery](), _q.inters)
}

// CountX is like Count, but panics if an error occurs.
func (_q *JudgeRubricQuery) CountX(ctx context.Context) int {
	count, err := _q.Count(ctx)
	if err != nil {
		panic(err)
	}
	return count
}

// Exist returns true if the query has elements in the graph.
func (_q *JudgeRubricQuery) Exist(ctx context.Context) (bool, error) {
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryExist)
	switch _, err := _q.FirstID(ctx); {
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent: check existence: %w", err)
	default:
		return true, nil
	}
}

// ExistX is like Exist, but panics if an error occurs.
func (_q *JudgeRubricQuery) ExistX(ctx context.Context) bool {
	exist, err := _q.Exist(ctx)
	if err != nil {
		panic(err)
	}
	return exist
}

// Clone returns a duplicate of the JudgeRubricQuery builder, including all associated steps. It can be
// used to prepare common query builders and use them differently after the clone is made.
func (_q *JudgeRubricQuery) Clone() *JudgeRubricQuery {
	if _q == nil {
		return nil
	}
	return &JudgeRubricQuery{
		config:       _q.config,
		ctx:          _q.ctx.Clone(),
		order:        append([]judgerubric.OrderOption{}, _q.order...),
		inters:       append([]Interceptor{}, _q.inters...),
		predicates:   append([]predicate.JudgeRubric{}, _q.predicates...),
		withWorkflow: _q.withWorkflow.Clone(),
		withResults:  _q.withResults.Clone(),
		// clone intermediate query.
		sql:  _q.sql.Clone(),
		path: _q.path,
	}
}

// WithWorkflow tells the query-builder to eager-load the nodes that are connected to
// the "workflow" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *JudgeRubricQuery) WithWorkflow(opts ...func(*SpecWorkflowQuery)) *JudgeRubricQuery {
	query := (&SpecWorkflowClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withWorkflow = query
	return _q
}

// WithResults tells the query-builder to eager-load the nodes that are connected to
// the "results" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *JudgeRubricQuery) WithResults(opts ...func(*JudgeResultQuery)) *JudgeRubricQuery {
	query := (&JudgeResultClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withResults = query
	return _q
}

// GroupBy is used to group vertices by one or more fields/columns.
// It is often used with aggregate functions, like: count, max, mean, min, sum.
//
// Example:
//
//	var v []struct {
//		SpecType string `json:"spec_type,omitempty"`
//		Count int `json:"count,omitempty"`
//	}
//
//	client.JudgeRubric.Query().
//		GroupBy(judgerubric.FieldSpecType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *JudgeRubricQuery) GroupBy(field string, fields ...string) *JudgeRubricGroupBy {
	_q.ctx.Fields = append([]string{field}, fields...)
	grbuild := &JudgeRubricGroupBy{build: _q}
	grbuild.flds = &_q.ctx.Fields
	grbuild.label = judgerubric.Label
	grbuild.scan = grbuild.Scan
	return grbuild
}

// Select allows the selection one or more fields/columns for the given query,
// instead of selecting all fields in the entity.
//
// Example:
//
//	var v []struct {
//		SpecType string `json:"spec_type,omitempty"`
//	}
//
//	client.JudgeRubric.Query().
//		Select(judgerubric.FieldSpecType).
//		Scan(ctx, &v)
func (_q *JudgeRubricQuery) Select(fields ...string) *JudgeRubricSelect {
	_q.ctx.Fields = append(_q.ctx.Fields, fields...)
	sbuild := &JudgeRubricSelect{JudgeRubricQuery: _q}
	sbuild.label = judgerubric.Label
	sbuild.flds, sbuild.scan = &_q.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a JudgeRubricSelect configured with the given aggregations.
func (_q *JudgeRubricQuery) Aggregate(fns ...AggregateFunc) *JudgeRubricSelect {
	return _q.Select().Aggregate(fns...)
}

func (_q *JudgeRubricQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range _q.inters {
		if inter == nil {
			return fmt.Errorf("ent: uninitialized interceptor (forgotten import ent/runtime?)")
		}
		if trv, ok := inter.(Traverser); ok {
			if err := trv.Traverse(ctx, _q); err != nil {
				return err
			}
		}
	}
	for _, f := range _q.ctx.Fields {
		if !judgerubric.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
		}
	}
	if _q.path != nil {
		prev, err := _q.path(ctx)
		if err != nil {
			return err
		}
		_q.sql = prev
	}
	return nil
}

func (_q *JudgeRubricQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*JudgeRubric, error) {
	var (
		nodes       = []*JudgeRubric{}
		withFKs     = _q.withFKs
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withWorkflow != nil,
			_q.withResults != nil,
		}
	)
	if _q.withWorkflow != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, judgerubric.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*JudgeRubric).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &JudgeRubric{config: _q.config}
		nodes = append(nodes, node)
		node.Edges.loadedTypes = loadedTypes
		return node.assignValues(columns, values)
	}
	for i := range hooks {
		hooks[i](ctx, _spec)
	}
	if err := sqlgraph.QueryNodes(ctx, _q.driver, _spec); err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nodes, nil
	}
	if query := _q.withWorkflow; query != nil {
		if err := _q.loadWorkflow(ctx, query, nodes, nil,
			func(n *JudgeRubric, e *SpecWorkflow) { n.Edges.Workflow = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withResults; query != nil {
		if err := _q.loadResults(ctx, query, nodes,
			func(n *JudgeRubric) { n.Edges.Results = []*JudgeResult{} },
			func(n *JudgeRubric, e *JudgeResult) { n.Edges.Results = append(n.Edges.Results, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

func (_q *JudgeRubricQuery) loadWorkflow(ctx context.Context, query *SpecWorkflowQuery, nodes []*JudgeRubric, init func(*JudgeRubric), assign func(*JudgeRubric, *SpecWorkflow)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*JudgeRubric)
	for i := range nodes {
		if nodes[i].spec_workflow_rubrics == nil {
			continue
		}
		fk := *nodes[i].spec_workflow_rubrics
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(specworkflow.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "spec_workflow_rubrics" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}
func (_q *JudgeRubricQuery) loadResults(ctx context.Context, query *JudgeResultQuery, nodes []*JudgeRubric, init func(*JudgeRubric), assign func(*JudgeRubric, *JudgeResult)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*JudgeRubric)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
		if init != nil {
			init(nodes[i])
		}
	}
	query.withFKs = true
	query.Where(predicate.JudgeResult(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(judgerubric.ResultsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		fk := n.judge_rubric_results
		if fk == nil {
			return fmt.Errorf(`foreign-key "judge_rubric_results" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "judge_rubric_results" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}

func (_q *JudgeRubricQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := _q.querySpec()
	_spec.Node.Columns = _q.ctx.Fields
	if len(_q.ctx.Fields) > 0 {
		_spec.Unique = _q.ctx.Unique != nil && *_q.ctx.Unique
	}
	return sqlgraph.CountNodes(ctx, _q.driver, _spec)
}

func (_q *JudgeRubricQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(judgerubric.Table, judgerubric.Columns, sqlgraph.NewFieldSpec(judgerubric.FieldID, field.TypeString))
	_spec.From = _q.sql
	if unique := _q.ctx.Unique; unique != nil {
		_spec.Unique = *unique
	} else if _q.path != nil {
		_spec.Unique = true
	}
	if fields := _q.ctx.Fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, judgerubric.FieldID)
		for i := range fields {
			if fields[i] != judgerubric.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
	}
	if ps := _q.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if limit := _q.ctx.Limit; limit != nil {
		_spec.Limit = *limit
	}
	if offset := _q.ctx.Offset; offset != nil {
		_spec.Offset = *offset
	}
	if ps := _q.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	return _spec
}

func (_q *JudgeRubricQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(_q.driver.Dialect())
	t1 := builder.Table(judgerubric.Table)
	columns := _q.ctx.Fields
	if len(columns) == 0 {
		columns = judgerubric.Columns
	}
	selector := builder.Select(t1.Columns(columns...)...).From(t1)
	if _q.sql != nil {
		selector = _q.sql
		selector.Select(selector.Columns(columns...)...)
	}
	if _q.ctx.Unique != nil && *_q.ctx.Unique {
		selector.Distinct()
	}
	for _, p := range _q.predicates {
		p(selector)
	}
	for _, p := range _q.order {
		p(selector)
	}
	if offset := _q.ctx.Offset; offset != nil {
		// limit is mandatory for offset clause. We start
		// with default value, and override it below if needed.
		selector.Offset(*offset).Limit(math.MaxInt32)
	}
	if limit := _q.ctx.Limit; limit != nil {
		selector.Limit(*limit)
	}
	return selector
}

// JudgeRubricGroupBy is the group-by builder for JudgeRubric entities.
type JudgeRubricGroupBy struct {
	selector
	build *JudgeRubricQuery
}

// Aggregate adds the given aggregation functions to the group-by query.
func (_g *JudgeRubricGroupBy) Aggregate(fns ...AggregateFunc) *JudgeRubricGroupBy {
	_g.fns = append(_g.fns, fns...)
	return _g
}

// Scan applies the selector query and scans the result into the given value.
func (_g *JudgeRubricGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, _g.build.ctx, ent.OpQueryGroupBy)
	if err := _g.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*JudgeRubricQuery, *JudgeRubricGroupBy](ctx, _g.build, _g, _g.build.inters, v)
}

func (_g *JudgeRubricGroupBy) sqlScan(ctx context.Context, root *JudgeRubricQuery, v any) error {
	selector := root.sqlQuery(ctx).Select()
	aggregation := make([]string, 0, len(_g.fns))
	for _, fn := range _g.fns {
		aggregation = append(aggregation, fn(selector))
	}
	if len(selector.SelectedColumns()) == 0 {
		columns := make([]string, 0, len(*_g.flds)+len(_g.fns))
		for _, f := range *_g.flds {
			columns = append(columns, selector.C(f))
		}
		columns = append(columns, aggregation...)
		selector.Select(columns...)
	}
	selector.GroupBy(selector.Columns(*_g.flds...)...)
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := _g.build.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}

// JudgeRubricSelect is the builder for selecting fields of JudgeRubric entities.
type JudgeRubricSelect struct {
	*JudgeRubricQuery
	selector
}

// Aggregate adds the given aggregation functions to the selector query.
func (_s *JudgeRubricSelect) Aggregate(fns ...AggregateFunc) *JudgeRubricSelect {
	_s.fns = append(_s.fns, fns...)
	return _s
}

// Scan applies the selector query and scans the result into the given value.
func (_s *JudgeRubricSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, _s.ctx, ent.OpQuerySelect)
	if err := _s.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*JudgeRubricQuery, *JudgeRubricSelect](ctx, _s.JudgeRubricQuery, _s, _s.inters, v)
}

func (_s *JudgeRubricSelect) sqlScan(ctx context.Context, root *JudgeRubricQuery, v any) error {
	selector := root.sqlQuery(ctx)
	aggregation := make([]string, 0, len(_s.fns))
	for _, fn := range _s.fns {
		aggregation = append(aggregation, fn(selector))
	}
	switch n := len(*_s.selector.flds); {
	case n == 0 && len(aggregation) > 0:
		selector.Select(aggregation...)
	case n != 0 && len(aggregation) > 0:
		selector.AppendSelect(aggregation...)
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := _s.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}
