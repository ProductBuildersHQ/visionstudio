...
type Organization struct {
	...
	// ReleasePageURL holds the value of the "release_page_url" field.
...
// was not loaded in eager-loading.
func (e OrganizationEdges) MembersOrErr() ([]*Person, error) {
	if e.loadedTypes[1] {
...
func (_m *Organization) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case organization.FieldReleasePageURL:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field release_page_url", values[i])
			} else if value.Valid {
				_m.ReleasePageURL = value.String
			}
...
func (_m *Organization) QueryMembers() *PersonQuery {
...
func (_m *Organization) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("release_page_url=")
	builder.WriteString(_m.ReleasePageURL)
	builder.WriteString(", ")
