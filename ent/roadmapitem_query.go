// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"database/sql/driver"
	"fmt"
	"math"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/ProductBuildersHQ/visionstudio/ent/assignment"
	"github.com/ProductBuildersHQ/visionstudio/ent/deliveryevidence"
	"github.com/ProductBuildersHQ/visionstudio/ent/initiative"
	"github.com/ProductBuildersHQ/visionstudio/ent/phase"
	"github.com/ProductBuildersHQ/visionstudio/ent/predicate"
	"github.com/ProductBuildersHQ/visionstudio/ent/repository"
	"github.com/ProductBuildersHQ/visionstudio/ent/roadmapitem"
)

// RoadmapItemQuery is the builder for querying RoadmapItem entities.
type RoadmapItemQuery struct {
	config
	ctx             *QueryContext
	order           []roadmapitem.OrderOption
	inters          []Interceptor
	predicates      []predicate.RoadmapItem
	withInitiative  *InitiativeQuery
	withPhase       *PhaseQuery
	withRepository  *RepositoryQuery
	withAssignments *AssignmentQuery
	withEvidence    *DeliveryEvidenceQuery
	withFKs         bool
	// intermediate query (i.e. traversal path).
	sql  *sql.Selector
	path func(context.Context) (*sql.Selector, error)
}

// Where adds a new predicate for the RoadmapItemQuery builder.
func (_q *RoadmapItemQuery) Where(ps ...predicate.RoadmapItem) *RoadmapItemQuery {
	_q.predicates = append(_q.predicates, ps...)
	return _q
}

// Limit the number of records to be returned by this query.
func (_q *RoadmapItemQuery) Limit(limit int) *RoadmapItemQuery {
	_q.ctx.Limit = &limit
	return _q
}

// Offset to start from.
func (_q *RoadmapItemQuery) Offset(offset int) *RoadmapItemQuery {
	_q.ctx.Offset = &offset
	return _q
}

// Unique configures the query builder to filter duplicate records on query.
// By default, unique is set to true, and can be disabled using this method.
func (_q *RoadmapItemQuery) Unique(unique bool) *RoadmapItemQuery {
	_q.ctx.Unique = &unique
	return _q
}

// Order specifies how the records should be ordered.
func (_q *RoadmapItemQuery) Order(o ...roadmapitem.OrderOption) *RoadmapItemQuery {
	_q.order = append(_q.order, o...)
	return _q
}

// QueryInitiative chains the current query on the "initiative" edge.
func (_q *RoadmapItemQuery) QueryInitiative() *InitiativeQuery {
	query := (&InitiativeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(roadmapitem.Table, roadmapitem.FieldID, selector),
			sqlgraph.To(initiative.Table, initiative.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, roadmapitem.InitiativeTable, roadmapitem.InitiativeColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryPhase chains the current query on the "phase" edge.
func (_q *RoadmapItemQuery) QueryPhase() *PhaseQuery {
	query := (&PhaseClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(roadmapitem.Table, roadmapitem.FieldID, selector),
			sqlgraph.To(phase.Table, phase.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, roadmapitem.PhaseTable, roadmapitem.PhaseColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryRepository chains the current query on the "repository" edge.
func (_q *RoadmapItemQuery) QueryRepository() *RepositoryQuery {
	query := (&RepositoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(roadmapitem.Table, roadmapitem.FieldID, selector),
			sqlgraph.To(repository.Table, repository.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, roadmapitem.RepositoryTable, roadmapitem.RepositoryColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryAssignments chains the current query on the "assignments" edge.
func (_q *RoadmapItemQuery) QueryAssignments() *AssignmentQuery {
	query := (&AssignmentClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(roadmapitem.Table, roadmapitem.FieldID, selector),
			sqlgraph.To(assignment.Table, assignment.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, roadmapitem.AssignmentsTable, roadmapitem.AssignmentsColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryEvidence chains the current query on the "evidence" edge.
func (_q *RoadmapItemQuery) QueryEvidence() *DeliveryEvidenceQuery {
	query := (&DeliveryEvidenceClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := _q.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(roadmapitem.Table, roadmapitem.FieldID, selector),
			sqlgraph.To(deliveryevidence.Table, deliveryevidence.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, roadmapitem.EvidenceTable, roadmapitem.EvidenceColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// First returns the first RoadmapItem entity from the query.
// Returns a *NotFoundError when no RoadmapItem was found.
func (_q *RoadmapItemQuery) First(ctx context.Context) (*RoadmapItem, error) {
	nodes, err := _q.Limit(1).All(setContextOp(ctx, _q.ctx, ent.OpQueryFirst))
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nil, &NotFoundError{roadmapitem.Label}
	}
	return nodes[0], nil
}

// FirstX is like First, but panics if an error occurs.
func (_q *RoadmapItemQuery) FirstX(ctx context.Context) *RoadmapItem {
	node, err := _q.First(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
}

// FirstID returns the first RoadmapItem ID from the query.
// Returns a *NotFoundError when no RoadmapItem ID was found.
func (_q *RoadmapItemQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
	if ids, err = _q.Limit(1).IDs(setContextOp(ctx, _q.ctx, ent.OpQueryFirstID)); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{roadmapitem.Label}
		return
	}
	return ids[0], nil
}

// FirstIDX is like FirstID, but panics if an error occurs.
func (_q *RoadmapItemQuery) FirstIDX(ctx context.Context) string {
	id, err := _q.FirstID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
}

// Only returns a single RoadmapItem entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one RoadmapItem entity is found.
// Returns a *NotFoundError when no RoadmapItem entities are found.
func (_q *RoadmapItemQuery) Only(ctx context.Context) (*RoadmapItem, error) {
	nodes, err := _q.Limit(2).All(setContextOp(ctx, _q.ctx, ent.OpQueryOnly))
	if err != nil {
		return nil, err
	}
	switch len(nodes) {
	case 1:
		return nodes[0], nil
	case 0:
		return nil, &NotFoundError{roadmapitem.Label}
	default:
		return nil, &NotSingularError{roadmapitem.Label}
	}
}

// OnlyX is like Only, but panics if an error occurs.
func (_q *RoadmapItemQuery) OnlyX(ctx context.Context) *RoadmapItem {
	node, err := _q.Only(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// OnlyID is like Only, but returns the only RoadmapItem ID in the query.
// Returns a *NotSingularError when more than one RoadmapItem ID is found.
// Returns a *NotFoundError when no entities are found.
func (_q *RoadmapItemQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
	if ids, err = _q.Limit(2).IDs(setContextOp(ctx, _q.ctx, ent.OpQueryOnlyID)); err != nil {
		return
	}
	switch len(ids) {
	case 1:
		id = ids[0]
	case 0:
		err = &NotFoundError{roadmapitem.Label}
	default:
		err = &NotSingularError{roadmapitem.Label}
	}
	return
}

// OnlyIDX is like OnlyID, but panics if an error occurs.
func (_q *RoadmapItemQuery) OnlyIDX(ctx context.Context) string {
	id, err := _q.OnlyID(ctx)
	if err != nil {
		panic(err)
	}
	return id
}

// All executes the query and returns a list of RoadmapItems.
func (_q *RoadmapItemQuery) All(ctx context.Context) ([]*RoadmapItem, error) {
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryAll)
	if err := _q.prepareQuery(ctx); err != nil {
		return nil, err
	}
	qr := querierAll[[]*RoadmapItem, *RoadmapItemQuery]()
	return withInterceptors[[]*RoadmapItem](ctx, _q, qr, _q.inters)
}

// AllX is like All, but panics if an error occurs.
func (_q *RoadmapItemQuery) AllX(ctx context.Context) []*RoadmapItem {
	nodes, err := _q.All(ctx)
	if err != nil {
		panic(err)
	}
	return nodes
}

// IDs executes the query and returns a list of RoadmapItem IDs.
func (_q *RoadmapItemQuery) IDs(ctx context.Context) (ids []string, err error) {
	if _q.ctx.Unique == nil && _q.path != nil {
		_q.Unique(true)
	}
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryIDs)
	if err = _q.Select(roadmapitem.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}

// IDsX is like IDs, but panics if an error occurs.
func (_q *RoadmapItemQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
	return ids
}

// Count returns the count of the given query.
func (_q *RoadmapItemQuery) Count(ctx context.Context) (int, error) {
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryCount)
	if err := _q.prepareQuery(ctx); err != nil {
		return 0, err
	}
	return withInterceptors[int](ctx, _q, querierCount[*RoadmapItemQuery](), _q.inters)
}

// CountX is like Count, but panics if an error occurs.
func (_q *RoadmapItemQuery) CountX(ctx context.Context) int {
	count, err := _q.Count(ctx)
	if err != nil {
		panic(err)
	}
	return count
}

// Exist returns true if the query has elements in the graph.
func (_q *RoadmapItemQuery) Exist(ctx context.Context) (bool, error) {
	ctx = setContextOp(ctx, _q.ctx, ent.OpQueryExist)
	switch _, err := _q.FirstID(ctx); {
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent: check existence: %w", err)
	default:
		return true, nil
	}
}

// ExistX is like Exist, but panics if an error occurs.
func (_q *RoadmapItemQuery) ExistX(ctx context.Context) bool {
	exist, err := _q.Exist(ctx)
	if err != nil {
		panic(err)
	}
	return exist
}

// Clone returns a duplicate of the RoadmapItemQuery builder, including all associated steps. It can be
// used to prepare common query builders and use them differently after the clone is made.
func (_q *RoadmapItemQuery) Clone() *RoadmapItemQuery {
	if _q == nil {
		return nil
	}
	return &RoadmapItemQuery{
		config:          _q.config,
		ctx:             _q.ctx.Clone(),
		order:           append([]roadmapitem.OrderOption{}, _q.order...),
		inters:          append([]Interceptor{}, _q.inters...),
		predicates:      append([]predicate.RoadmapItem{}, _q.predicates...),
		withInitiative:  _q.withInitiative.Clone(),
		withPhase:       _q.withPhase.Clone(),
		withRepository:  _q.withRepository.Clone(),
		withAssignments: _q.withAssignments.Clone(),
		withEvidence:    _q.withEvidence.Clone(),
		// clone intermediate query.
		sql:  _q.sql.Clone(),
		path: _q.path,
	}
}

// WithInitiative tells the query-builder to eager-load the nodes that are connected to
// the "initiative" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoadmapItemQuery) WithInitiative(opts ...func(*InitiativeQuery)) *RoadmapItemQuery {
	query := (&InitiativeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInitiative = query
	return _q
}

// WithPhase tells the query-builder to eager-load the nodes that are connected to
// the "phase" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoadmapItemQuery) WithPhase(opts ...func(*PhaseQuery)) *RoadmapItemQuery {
	query := (&PhaseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPhase = query
	return _q
}

// WithRepository tells the query-builder to eager-load the nodes that are connected to
// the "repository" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoadmapItemQuery) WithRepository(opts ...func(*RepositoryQuery)) *RoadmapItemQuery {
	query := (&RepositoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRepository = query
	return _q
}

// WithAssignments tells the query-builder to eager-load the nodes that are connected to
// the "assignments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoadmapItemQuery) WithAssignments(opts ...func(*AssignmentQuery)) *RoadmapItemQuery {
	query := (&AssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAssignments = query
	return _q
}

// WithEvidence tells the query-builder to eager-load the nodes that are connected to
// the "evidence" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoadmapItemQuery) WithEvidence(opts ...func(*DeliveryEvidenceQuery)) *RoadmapItemQuery {
	query := (&DeliveryEvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEvidence = query
	return _q
}

// GroupBy is used to group vertices by one or more fields/columns.
// It is often used with aggregate functions, like: count, max, mean, min, sum.
//
// Example:
//
//	var v []struct {
//		Title string `json:"title,omitempty"`
//		Count int `json:"count,omitempty"`
//	}
//
//	client.RoadmapItem.Query().
//		GroupBy(roadmapitem.FieldTitle).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *RoadmapItemQuery) GroupBy(field string, fields ...string) *RoadmapItemGroupBy {
	_q.ctx.Fields = append([]string{field}, fields...)
	grbuild := &RoadmapItemGroupBy{build: _q}
	grbuild.flds = &_q.ctx.Fields
	grbuild.label = roadmapitem.Label
	grbuild.scan = grbuild.Scan
	return grbuild
}

// Select allows the selection one or more fields/columns for the given query,
// instead of selecting all fields in the entity.
//
// Example:
//
//	var v []struct {
//		Title string `json:"title,omitempty"`
//	}
//
//	client.RoadmapItem.Query().
//		Select(roadmapitem.FieldTitle).
//		Scan(ctx, &v)
func (_q *RoadmapItemQuery) Select(fields ...string) *RoadmapItemSelect {
	_q.ctx.Fields = append(_q.ctx.Fields, fields...)
	sbuild := &RoadmapItemSelect{RoadmapItemQuery: _q}
	sbuild.label = roadmapitem.Label
	sbuild.flds, sbuild.scan = &_q.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a RoadmapItemSelect configured with the given aggregations.
func (_q *RoadmapItemQuery) Aggregate(fns ...AggregateFunc) *RoadmapItemSelect {
	return _q.Select().Aggregate(fns...)
}

func (_q *RoadmapItemQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range _q.inters {
		if inter == nil {
			return fmt.Errorf("ent: uninitialized interceptor (forgotten import ent/runtime?)")
		}
		if trv, ok := inter.(Traverser); ok {
			if err := trv.Traverse(ctx, _q); err != nil {
				return err
			}
		}
	}
	for _, f := range _q.ctx.Fields {
		if !roadmapitem.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
		}
	}
	if _q.path != nil {
		prev, err := _q.path(ctx)
		if err != nil {
			return err
		}
		_q.sql = prev
	}
	return nil
}

func (_q *RoadmapItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RoadmapItem, error) {
	var (
		nodes       = []*RoadmapItem{}
		withFKs     = _q.withFKs
		_spec       = _q.querySpec()
		loadedTypes = [5]bool{
			_q.withInitiative != nil,
			_q.withPhase != nil,
			_q.withRepository != nil,
			_q.withAssignments != nil,
			_q.withEvidence != nil,
		}
	)
	if _q.withInitiative != nil || _q.withPhase != nil || _q.withRepository != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, roadmapitem.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*RoadmapItem).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &RoadmapItem{config: _q.config}
		nodes = append(nodes, node)
		node.Edges.loadedTypes = loadedTypes
		return node.assignValues(columns, values)
	}
	for i := range hooks {
		hooks[i](ctx, _spec)
	}
	if err := sqlgraph.QueryNodes(ctx, _q.driver, _spec); err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nodes, nil
	}
	if query := _q.withInitiative; query != nil {
		if err := _q.loadInitiative(ctx, query, nodes, nil,
			func(n *RoadmapItem, e *Initiative) { n.Edges.Initiative = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPhase; query != nil {
		if err := _q.loadPhase(ctx, query, nodes, nil,
			func(n *RoadmapItem, e *Phase) { n.Edges.Phase = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRepository; query != nil {
		if err := _q.loadRepository(ctx, query, nodes, nil,
			func(n *RoadmapItem, e *Repository) { n.Edges.Repository = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAssignments; query != nil {
		if err := _q.loadAssignments(ctx, query, nodes,
			func(n *RoadmapItem) { n.Edges.Assignments = []*Assignment{} },
			func(n *RoadmapItem, e *Assignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvidence; query != nil {
		if err := _q.loadEvidence(ctx, query, nodes,
			func(n *RoadmapItem) { n.Edges.Evidence = []*DeliveryEvidence{} },
			func(n *RoadmapItem, e *DeliveryEvidence) { n.Edges.Evidence = append(n.Edges.Evidence, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

func (_q *RoadmapItemQuery) loadInitiative(ctx context.Context, query *InitiativeQuery, nodes []*RoadmapItem, init func(*RoadmapItem), assign func(*RoadmapItem, *Initiative)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*RoadmapItem)
	for i := range nodes {
		if nodes[i].initiative_roadmap_items == nil {
			continue
		}
		fk := *nodes[i].initiative_roadmap_items
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(initiative.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "initiative_roadmap_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}
func (_q *RoadmapItemQuery) loadPhase(ctx context.Context, query *PhaseQuery, nodes []*RoadmapItem, init func(*RoadmapItem), assign func(*RoadmapItem, *Phase)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*RoadmapItem)
	for i := range nodes {
		if nodes[i].phase_roadmap_items == nil {
			continue
		}
		fk := *nodes[i].phase_roadmap_items
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(phase.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "phase_roadmap_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}
func (_q *RoadmapItemQuery) loadRepository(ctx context.Context, query *RepositoryQuery, nodes []*RoadmapItem, init func(*RoadmapItem), assign func(*RoadmapItem, *Repository)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*RoadmapItem)
	for i := range nodes {
		if nodes[i].repository_roadmap_items == nil {
			continue
		}
		fk := *nodes[i].repository_roadmap_items
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(repository.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "repository_roadmap_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}
func (_q *RoadmapItemQuery) loadAssignments(ctx context.Context, query *AssignmentQuery, nodes []*RoadmapItem, init func(*RoadmapItem), assign func(*RoadmapItem, *Assignment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*RoadmapItem)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
		if init != nil {
			init(nodes[i])
		}
	}
	query.withFKs = true
	query.Where(predicate.Assignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(roadmapitem.AssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		fk := n.roadmap_item_assignments
		if fk == nil {
			return fmt.Errorf(`foreign-key "roadmap_item_assignments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "roadmap_item_assignments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *RoadmapItemQuery) loadEvidence(ctx context.Context, query *DeliveryEvidenceQuery, nodes []*RoadmapItem, init func(*RoadmapItem), assign func(*RoadmapItem, *DeliveryEvidence)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*RoadmapItem)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
		if init != nil {
			init(nodes[i])
		}
	}
	query.withFKs = true
	query.Where(predicate.DeliveryEvidence(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(roadmapitem.EvidenceColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		fk := n.roadmap_item_evidence
		if fk == nil {
			return fmt.Errorf(`foreign-key "roadmap_item_evidence" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "roadmap_item_evidence" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}

func (_q *RoadmapItemQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := _q.querySpec()
	_spec.Node.Columns = _q.ctx.Fields
	if len(_q.ctx.Fields) > 0 {
		_spec.Unique = _q.ctx.Unique != nil && *_q.ctx.Unique
	}
	return sqlgraph.CountNodes(ctx, _q.driver, _spec)
}

func (_q *RoadmapItemQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(roadmapitem.Table, roadmapitem.Columns, sqlgraph.NewFieldSpec(roadmapitem.FieldID, field.TypeString))
	_spec.From = _q.sql
	if unique := _q.ctx.Unique; unique != nil {
		_spec.Unique = *unique
	} else if _q.path != nil {
		_spec.Unique = true
	}
	if fields := _q.ctx.Fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, roadmapitem.FieldID)
		for i := range fields {
			if fields[i] != roadmapitem.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
	}
	if ps := _q.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if limit := _q.ctx.Limit; limit != nil {
		_spec.Limit = *limit
	}
	if offset := _q.ctx.Offset; offset != nil {
		_spec.Offset = *offset
	}
	if ps := _q.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	return _spec
}

func (_q *RoadmapItemQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(_q.driver.Dialect())
	t1 := builder.Table(roadmapitem.Table)
	columns := _q.ctx.Fields
	if len(columns) == 0 {
		columns = roadmapitem.Columns
	}
	selector := builder.Select(t1.Columns(columns...)...).From(t1)
	if _q.sql != nil {
		selector = _q.sql
		selector.Select(selector.Columns(columns...)...)
	}
	if _q.ctx.Unique != nil && *_q.ctx.Unique {
		selector.Distinct()
	}
	for _, p := range _q.predicates {
		p(selector)
	}
	for _, p := range _q.order {
		p(selector)
	}
	if offset := _q.ctx.Offset; offset != nil {
		// limit is mandatory for offset clause. We start
		// with default value, and override it below if needed.
		selector.Offset(*offset).Limit(math.MaxInt32)
	}
	if limit := _q.ctx.Limit; limit != nil {
		selector.Limit(*limit)
	}
	return selector
}

// RoadmapItemGroupBy is the group-by builder for RoadmapItem entities.
type RoadmapItemGroupBy struct {
	selector
	build *RoadmapItemQuery
}

// Aggregate adds the given aggregation functions to the group-by query.
func (_g *RoadmapItemGroupBy) Aggregate(fns ...AggregateFunc) *RoadmapItemGroupBy {
	_g.fns = append(_g.fns, fns...)
	return _g
}

// Scan applies the selector query and scans the result into the given value.
func (_g *RoadmapItemGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, _g.build.ctx, ent.OpQueryGroupBy)
	if err := _g.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*RoadmapItemQuery, *RoadmapItemGroupBy](ctx, _g.build, _g, _g.build.inters, v)
}

func (_g *RoadmapItemGroupBy) sqlScan(ctx context.Context, root *RoadmapItemQuery, v any) error {
	selector := root.sqlQuery(ctx).Select()
	aggregation := make([]string, 0, len(_g.fns))
	for _, fn := range _g.fns {
		aggregation = append(aggregation, fn(selector))
	}
	if len(selector.SelectedColumns()) == 0 {
		columns := make([]string, 0, len(*_g.flds)+len(_g.fns))
		for _, f := range *_g.flds {
			columns = append(columns, selector.C(f))
		}
		columns = append(columns, aggregation...)
		selector.Select(columns...)
	}
	selector.GroupBy(selector.Columns(*_g.flds...)...)
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := _g.build.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}

// RoadmapItemSelect is the builder for selecting fields of RoadmapItem entities.
type RoadmapItemSelect struct {
	*RoadmapItemQuery
	selector
}

// Aggregate adds the given aggregation functions to the selector query.
func (_s *RoadmapItemSelect) Aggregate(fns ...AggregateFunc) *RoadmapItemSelect {
	_s.fns = append(_s.fns, fns...)
	return _s
}

// Scan applies the selector query and scans the result into the given value.
func (_s *RoadmapItemSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, _s.ctx, ent.OpQuerySelect)
	if err := _s.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*RoadmapItemQuery, *RoadmapItemSelect](ctx, _s.RoadmapItemQuery, _s, _s.inters, v)
}

func (_s *RoadmapItemSelect) sqlScan(ctx context.Context, root *RoadmapItemQuery, v any) error {
	selector := root.sqlQuery(ctx)
	aggregation := make([]string, 0, len(_s.fns))
	for _, fn := range _s.fns {
		aggregation = append(aggregation, fn(selector))
	}
	switch n := len(*_s.selector.flds); {
	case n == 0 && len(aggregation) > 0:
		selector.Select(aggregation...)
	case n != 0 && len(aggregation) > 0:
		selector.AppendSelect(aggregation...)
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := _s.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}
