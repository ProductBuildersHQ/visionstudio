...
import type { SpendResponse, ExecutionResponse, APIRMI } from '../api/types'
...
export function SpendPanel() {
  ...
  if (!spend || !execution) {
    return <LoadingState message="Loading spend data..." />
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">Spend & Accomplishments</h2>
      ...
      {/* Cost by Initiative */}
      <CostByInitiative spend={spend} execution={execution} />
    </div>
  )
}
            ...
            <div className="space-y-2">
              {group.items.map((rmi) => (
                <AccomplishmentRow
                  ...
                  cost={spend.byRmi?.[rmi.id]?.costUsd}
                />
              ))}
            </div>
          </div>
